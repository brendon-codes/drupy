...
def drupal_process_form(form_id, &form, &form_state):
  ...
  if ((not empty(form['#programmed'])) or (not empty(form['#post']) and (isset(form['#post']['form_id']) and (form['#post']['form_id'] == form_id)))):
    ...
    if ((not empty(form_state['submitted'])) and not form_get_errors() and empty(form_state['rebuild'])):
      form_state['redirect'] = None
      ...
      # drupal_execute).
      if (batch =& batch_get() and not isset(batch['current_set'])):
...
def drupal_redirect_form(form, redirect = None):
  ...
  if (not isset(goto) or (goto !== False)):
    if (isset(goto)):
      if (is_array(goto)):
...
def expand_password_confirm(element):
  element['pass1'] =  array(
    '#type' : 'password',
    '#title' : t('Password'),
    '#value' : empty(element['#value']) ? None : element['#value']['pass1'],
  ...
  element['pass2'] =  array(
    ...
    '#value' : empty(element['#value']) ? None : element['#value']['pass2'],
    '#required' : element['#required'],
