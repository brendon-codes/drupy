      ...
      if (not isset(data)):
        ...
        if (item['access']):
          ...
          if (in_array(menu_name, expanded)):
            ...
            while(num_rows):
              result = db_query("SELECT mlid FROM {menu_links} WHERE menu_name = '%s' AND expanded = 1 AND has_children = 1 AND plid IN (" + placeholders + ') AND mlid NOT IN (' + placeholders + ')', array_merge([menu_name], args, args))
              num_rows = False
...
def menu_tree_collect_node_links(tree, node_links):
  ...
  for key,v in tree.items():
    if (tree[key]['link']['router_path'] == 'node/%'):
      ...
      if (is_numeric(nid)):
        node_links[nid][tree[key]['link']['mlid']] = DrupyHelper.Reference(tree[key]['link'])
...
#
def menu_tree_check_access(tree, node_links = dict()):
  DrupyHelper.Reference.check(tree)
  if (node_links):
...
def _menu_tree_check_access(tree):
  ...
  new_tree = dict()
  for key,v in tree.items():
    item = DrupyHelper.Reference(tree[key]['link'])
...
def _menu_router_build(callbacks):
  ...
  for path,v in menu.items():
    item = DrupyHelper.Reference(menu[path])
    if (not item['_tab']):
      # Non-tab items.
      item['tab_parent'] = ''
      item['tab_root'] = path
