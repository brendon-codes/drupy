...
#
def file_save_upload(source, validators = array(), dest = False, replace = FILE_EXISTS_RENAME):
  global user
  static upload_cache
...
def file_validate_is_image(&file):
  ...
  if (not info or empty(info['extension'])):
  ...
  if (info = image_get_info(file.filepath)):
    if (maximum_dimensions):
      ...
      if (info['width'] > width or info['height'] > height):
        ...
        if (image_get_toolkit() and image_scale(file.filepath, file.filepath, width, height)):
...
#   matching files.
#
def file_scan_directory(dir, mask, nomask = array('.', '..', 'CVS'), callback = 0, recurse = True, key = 'filename', min_depth = 0, depth = 0):
